<?php
// 🏰 SECCIÓN: INTRODUCCIÓN - CASTILLO DE BLOIS
$introduccion = [
  "id" => "introduccion",
  "titulo" => "🏰 Descubre el Castillo de Blois",
"parrafos" => [
  "El <strong>Castillo Real de Blois</strong>, situado en el corazón del <strong>Valle del Loira</strong>, es una joya arquitectónica que reúne en un solo lugar cuatro estilos distintos: <strong>gótico, renacentista, clásico y barroco</strong>. Esta diversidad refleja la evolución de la monarquía francesa y la historia del arte entre los siglos XIII y XVII.",

  "Residencia favorita de <strong>siete reyes y diez reinas de Francia</strong>, el castillo fue escenario de intrigas políticas, conspiraciones cortesanas y episodios clave en la historia del reino. Entre sus muros se desarrollaron los grandes acontecimientos del Renacimiento francés.",

  "Su fachada principal, dominada por la elegante <strong>escalera renacentista</strong> encargada por <strong>Francisco I</strong>, es uno de los ejemplos más refinados de la arquitectura del siglo XVI. Cada ala del castillo cuenta una historia diferente, ofreciendo un auténtico recorrido por siglos de arte y poder.",

  "En el interior, las salas restauradas exhiben tapices, retratos reales y mobiliario de época. Destaca la <strong>cámara de Catalina de Médici</strong>, donde se conservan objetos asociados a las leyendas y misterios que rodean su figura.",

  "El castillo domina la ciudad de <strong>Blois</strong> y ofrece una vista magnífica sobre el Loira. Su entorno urbano combina historia, cultura y vida contemporánea, haciendo de este lugar una visita imprescindible para comprender el esplendor del Renacimiento francés.",

  "Hoy, el <strong>Castillo Real de Blois</strong> es Patrimonio Mundial de la <strong>UNESCO</strong> y acoge exposiciones, espectáculos de luz y sonido, y actividades culturales que permiten revivir la grandeza de la corte francesa en pleno siglo XXI."
]

];



$titulo = "📸 Galería";
$texto = "Imágenes del histórico Castillo de Blois, mostrando su arquitectura renacentista y sus majestuosos salones.";

$figuras = [
  [
    "src" => "https://woody.cloudly.space/app/uploads/blois-tourisme/2024/06/thumbs/ChateauRoyaldeBlois-5-1920x640.webp",
    "alt" => "Vista aerea del Castillo de Blois",
    "pie" => "Vista aerea del Castillo de Blois",
    "fuente_url" => "https://www.bloischambord.es/descubre-el-destino/castillos-del-loira/castillo-de-blois/",
    "fuente_texto" => "bloischambord.es"
  ],
  [
    "src" => "https://cdn-imgix.headout.com/media/images/960c259331f5665593d368745fdcc7cb-10508-RoyalBloisCastleSkip-the-LineTickets----006.jpg?auto=format&w=900&h=562.5&q=90&ar=16%3A10&crop=faces%2Ccenter&fit=crop",
    "alt" => "Interior del Castillo de Blois",
    "pie" => "Interior del Castillo de Blois",
    "fuente_url" => "https://www.loirevalley-tickets.com/es/chateau-de-blois/",
    "fuente_texto" => "loirevalley-tickets.com"
  ],
    [
    "src" => "https://d397xw3titc834.cloudfront.net/images/original/d/9c/d9c763a560566215cc4820ea7b0d3bda.jpg",
    "alt" => "Interior del Castillo de Blois",
    "pie" => "Salones renacentistas",
    "fuente_url" => "https://www.hotel-france-guise.com/es/a-descubrir-la-ciudad-de-blois/castillo-real-de-blois-4610",
    "fuente_texto" => "hotel-france-guise.com"
  ],
  [
    "src" => "https://www.jardinez.com/parc/pj/41jardinchatblois.jpg",
    "alt" => "Jardines del Castillo de Blois",
    "pie" => "Jardines históricos",
    "fuente_url" => "https://www.jardinez.com/Parques-Los-Jardines-del-Castillo-de-Blois_sp_300",
    "fuente_texto" => "jardinez.com"
  ],  [
    "src" => "https://www.cometoparis.com/data/layout_grouping/product_index_slideshow/56017_chateau-royal-de-blois-f-christophe.1000w.jpg?ver=1675417849",
    "alt" => "Vista general del Castillo de Blois",
    "pie" => "Vista aérea del castillo",
    "fuente_url" => "https://www.cometoparis.com/spa/castillos-del-loira/castillo-real-de-blois-m9000873",
    "fuente_texto" => "cometoparis.com"
  ],
  [
    "src" => "https://a.travel-assets.com/findyours-php/viewfinder/images/res70/132000/132715-Chateau-De-Blois.jpg",
    "alt" => "Patio del Castillo de Blois",
    "pie" => "Patio central",
    "fuente_url" => "https://www.expedia.es/Castillo-De-Blois-Blois.d6107672.Puntos-de-Interes",
    "fuente_texto" => "expedia.es"
  ],
  [
    "src" => "https://media.istockphoto.com/id/579422052/es/foto/el-castillo-real-de-blois-iluminado-por-la-noche.jpg?s=612x612&w=0&k=20&c=ejm7rd0wC3BM1ia1hdW-fXhXdxa9Ry7jq1Plv9bYtGA=",
    "alt" => "Escalera renacentista del Castillo de Blois",
    "pie" => "Escalera renacentista del castillo iluminado por la noche",
    "fuente_url" => "https://www.istockphoto.com/es/fotos/castillo-de-blois-fotos",
    "fuente_texto" => "istockphoto.com"
  ]
  ];
?>






