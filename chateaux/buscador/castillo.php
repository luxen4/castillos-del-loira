        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2 bg-white rounded shadow overflow-hidden">
                <div class="w-full bg-black">
                    <img src="<?= htmlspecialchars($found['src']) ?>"
                        alt="<?= htmlspecialchars($found['alt']) ?>"
                        class="w-full max-h-[400px] md:max-h-[700px] object-contain mx-auto bg-black">
                </div>

                <div class="p-4">
                    <p class="text-gray-600"><?= htmlspecialchars($found['short_description']) ?></p>
                    <p class="mt-3 text-sm text-gray-500">
                        📍 <?= htmlspecialchars($found['location']) ?> • Región: <?= htmlspecialchars($found['region']) ?> • Año: <?= htmlspecialchars($found['year']) ?>
                    </p>
                </div>
            </div>
