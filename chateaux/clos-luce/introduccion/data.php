<?php
// 🏰 SECCIÓN: INTRODUCCIÓN - CASTILLO DE CLOS-LUCÉ
$introduccion = [
  "id" => "introduccion",
  "titulo" => "🏰 Descubre el Castillo de Clos-Lucé",
"parrafos" => [
  "El <strong>Castillo de Clos-Lucé</strong>, situado en <strong>Amboise</strong>, es célebre por haber sido la última residencia de <strong>Leonardo da Vinci</strong>. En este entorno sereno, el genio del Renacimiento italiano pasó sus últimos años trabajando al servicio de <strong>Francisco I de Francia</strong>.",

  "Construido en el siglo XV, el castillo combina el encanto de la arquitectura gótica con elementos renacentistas, reflejando la transición artística de la época y la influencia italiana en el Valle del Loira.",

  "En su interior se conservan maquetas, bocetos y reconstrucciones de las invenciones de Leonardo, que permiten comprender su legado científico y artístico de una forma interactiva y educativa.",

  "Los jardines de Clos-Lucé, inspirados en los dibujos del maestro, albergan reproducciones a tamaño real de sus ingenios mecánicos, invitando a los visitantes a descubrir la mente creativa de uno de los mayores genios de la historia.",

  "Más que un castillo, Clos-Lucé es un lugar de inspiración y conocimiento, donde arte, ciencia y naturaleza se unen para rendir homenaje al espíritu del <strong>Renacimiento</strong>.",

  "Hoy en día, es una de las visitas imprescindibles del <strong>Valle del Loira</strong>, un espacio que combina historia, innovación y el legado universal de <strong>Leonardo da Vinci</strong>."
]

];
// 📸 SECCIÓN: GALERÍA DEL CASTILLO DE CLOS-LUCÉ
$titulo = "📸 Galería";
$texto = "Imágenes del Castillo de Clos-Lucé, última residencia de Leonardo da Vinci, mostrando su historia, talleres y jardines.";

$figuras = [
    [
    "src" => "https://woody.cloudly.space/app/uploads/blois-tourisme/2024/07/thumbs/Ateliers-de-leonard-de-vinci-Leonard-de-Serres-1-640x640.webp",
    "alt" => "Taller de Leonardo da Vinci en Clos-Lucé",
    "pie" => "Taller de Leonardo",
    "fuente_url" => "https://www.bloischambord.es/descubre-el-destino/castillos-del-loira/el-chateau-du-clos-luce/",
    "fuente_texto" => "bloischambord.es"
    ],  [
    "src" => "https://es.derutaenfamilia.com/wp-content/uploads/sites/2/2022/04/castillo_clos_luce_derutaenfamilia_004.jpg",
    "alt" => "Jardines del Castillo de Clos-Lucé",
    "pie" => "Jardines del Castillo de Clos-Lucé",
    "fuente_url" => "https://es.derutaenfamilia.com/castillo-clos-luce/",
    "fuente_texto" => "derutaenfamilia.com"
  ],

  [
    "src" => "https://es.derutaenfamilia.com/wp-content/uploads/sites/2/2022/04/mapa_clos_luce.jpg",
    "alt" => "Plano del parque del Castillo de Clos-Lucé",
    "pie" => "Plano del parque del Castillo de Clos-Lucé",
    "fuente_url" => "https://es.derutaenfamilia.com/castillo-clos-luce/",
    "fuente_texto" => "derutaenfamilia.com"
  ]
  ]
?>

